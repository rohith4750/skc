...
async function cleanupMenuItems() {
  ...
  for (const dbItem of dbItems) {
    ...
    if (!dataItem) {
      console.log(
        `Item "${dbItem.name}" exists in DB but not in data file - keeping as is`,
      );
      continue;
    }

    // Check if type needs to be updated
    const hasType = dbItem.type.includes(dataItem.type);
    if (!hasType || dbItem.type.length !== 1) {
      console.log(
        `Updating "${dbItem.name}": [${(dbItem.type as any).join(", ")}] -> [${dataItem.type}]`,
      );
      await prisma.menuItem.update({
        where: { id: dbItem.id },
        data: {
          type: [dataItem.type] as any,
    ...
    if (items.length > 1) {
      ...
      if (dataItem) {
        ...
        for (const item of items) {
          if (!item.type.includes(dataItem.type) || item.type.length !== 1) {
            console.log(
              `  Deleting duplicate "${item.name}" with type "[${(item.type as any).join(", ")}]" (should be "[${dataItem.type}]")`,
            );
