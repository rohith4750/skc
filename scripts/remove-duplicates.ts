  ...
  try {
    ...
    for (const item of allItems) {
      const typeStr = Array.isArray(item.type)
        ? item.type.sort().join(",")
        : String(item.type);
      const key = `${typeStr}|||${item.name.toLowerCase().trim()}`;
    ...
    for (const group of duplicateGroups) {
      ...
      const typeStr = Array.isArray(group[0].type)
        ? group[0].type.join(", ")
        : String(group[0].type);
      console.log(
        `   ${typeStr} - "${group[0].name}" (${group.length} copies)`,
      );
    }
    ...
    for (const group of duplicateGroups) {
      ...
      const keepTypeStr = Array.isArray(keep.type)
        ? keep.type.join(", ")
        : String(keep.type);
      console.log(`Keeping: ${keepTypeStr} - "${keep.name}" (ID: ${keep.id})`);
