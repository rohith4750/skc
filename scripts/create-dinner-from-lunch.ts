...
async function createDinnerItems() {
  ...
  try {
    ...
    console.log(
      `Found ${lunchItems.length} lunch items to duplicate for dinner\n`,
    );
    ...
    if (existingDinnerItems.length > 0) {
      console.log(
        `⚠️  Found ${existingDinnerItems.length} existing dinner items`,
      );
      ...
      try {
        await prisma.menuItem.create({
          data: {
            ...
            type: ["dinner"] as any, // Change type to dinner
        ...
        errors++;
        console.error(
          `❌ Failed to create dinner item for "${lunchItem.name}":`,
          error.message,
        );
      }
    }
    ...
    console.log(
      "   • Lunch items: All lunch/dinner items (sweets, dal, rice, curries, etc.)",
    );
