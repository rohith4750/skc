...
function groupBySubcategory(items: { name: string; nameTelugu?: string | null; description?: string | null }[]) {
...
function generateMenuPDF(menuItems: { name: string; nameTelugu?: string | null; type: string; description?: string | null }[]): Buffer {
  ...
  const addHeader = () => {
    ...
    doc.text('SRIVATSASA & KOWNDINYA CATERERS', pageWidth / 2, y, { align: 'center' })
    y += 6
    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    doc.text('Pure Vegetarian Catering - Complete Menu', pageWidth / 2, y, { align: 'center' })
    y += 12
  }
  ...
  const breakfast = menuItems.filter((i) => i.type === 'breakfast')
  const lunch = menuItems.filter((i) => i.type === 'lunch')
  const dinner = menuItems.filter((i) => i.type === 'dinner')
  const snacks = menuItems.filter((i) => i.type === 'snacks' || i.type === 'sweets')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const html = generateMenuEmailHTML({
      ...
      introMessage: data.introMessage || 'Please find our complete menu attached for your reference.',
    })
