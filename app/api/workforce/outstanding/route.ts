...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (date) {
      ...
      paymentWhere.paymentDate = { gte: startDate, lte: endDate };
    ...
    expenses.forEach((exp: any) => {
      ...
      if (exp.isBulkExpense && exp.bulkAllocations) {
        (exp.bulkAllocations as any[]).forEach(
          (a: any) => a.orderId && orderIdsSet.add(a.orderId),
        );
      }
    });
    ...
    expenses.forEach((exp: any) => {
      ...
      if (exp.orderId) {
        ...
        (exp.bulkAllocations as any[]).forEach((alloc: any) => {
          ...
          if (oid) {
            if (!eventsMap[oid])
              eventsMap[oid] = { expenses: [], totalAmount: 0 };
    ...
    workforcePayments.forEach((p: any) => {
      const role =
        p.role && WORKFORCE_ROLES.includes(p.role.toLowerCase())
          ? p.role.toLowerCase()
          : "other";
      if (!byRole[role])
        byRole[role] = {
