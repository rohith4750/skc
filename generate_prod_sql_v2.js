...
async function main() {
  ...
  let sql = `-- FOOD MENU REPORT MIGRATION FOR CUSTOMER: Manikyarao Behara Garu (Kesariii)
...
`;
  orders.forEach(order => {
    ...
    sql += `INSERT INTO orders (id, "serialNumber", "customerId", "eventName", "eventType", "eventDate", "totalAmount", status, "remainingAmount", "advancePaid", "createdAt", "updatedAt")
VALUES ('${order.id}', ${order.serialNumber}, '${order.customerId}', '${order.eventName.replace(/'/g, "''")}', '${order.eventType.replace(/'/g, "''")}', '${eventDate}', ${order.totalAmount}, 'pending', ${order.remainingAmount}, 0, NOW(), NOW());
...
`;
  ...
  fs.writeFileSync('production_menu_update.sql', sql);
  console.log('SQL generated in production_menu_update.sql');
}
